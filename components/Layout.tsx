...
import { Home, BookOpen, Music, Zap, Settings } from 'lucide-react';
    ...
    <div className="h-screen w-full bg-paper flex flex-col md:flex-row text-ink transition-colors duration-300">
      ...
      <aside className="hidden md:flex flex-col w-72 bg-surface dark:bg-stone-900 border-r border-stone-100 dark:border-stone-800 h-full p-6 shadow-soft z-20">
        <div className="mb-10 flex items-center gap-3 px-2">
          <div className="w-8 h-8 bg-gradient-to-br from-gold to-orange rounded-lg"></div>
          <h1 className="text-2xl font-serif font-bold text-ink dark:text-white tracking-tight">Lumina</h1>
        </div>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-paper/80 dark:bg-stone-950/80 backdrop-blur-md z-20 flex items-center justify-between px-6 border-b border-stone-100/50 dark:border-stone-800/50">
        <h1 className="text-xl font-serif font-bold text-ink dark:text-white">Lumina</h1>
        <Link to="/settings">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-surface dark:bg-stone-900 border-t border-stone-100 dark:border-stone-800 px-6 py-4 flex justify-between items-center z-30 shadow-[0_-4px_20px_-2px_rgba(0,0,0,0.05)] pb-safe">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              `}>
                <item.icon size={24} strokeWidth={active ? 2.5 : 2} fill={active ? "currentColor" : "none"} />
              </div>
              <span className={`text-[10px] font-medium transition-colors ${active ? 'text-ink dark:text-white font-bold' : 'text-stone-400'}`}>
                {item.label}
              </span>
            </Link>
          );
