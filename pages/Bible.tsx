...
import { ChevronLeft, ChevronRight, CheckCircle, X, BookOpen, Loader2, Type, Minus, Plus, Highlighter, Bookmark, RefreshCw } from 'lucide-react';
...
const Bible: React.FC = () => {
    ...
    if (saved) {
      ...
      if (!p.readChapters.includes(enId)) {
        const newProgress = {
          ...
          // Ensure lastRead is also updated here
          lastRead: enId 
        };
        localStorage.setItem('lumina_progress', JSON.stringify(newProgress));
        setIsRead(true);
  ...
  return (
    <div className="h-full flex flex-col relative book-perspective">
      ...
      {/* Toast Notification */}
      {showToast && (
        <div className="absolute top-20 left-1/2 -translate-x-1/2 z-50 bg-ink text-white px-6 py-3 rounded-full shadow-2xl animate-fade-in flex items-center gap-2">
